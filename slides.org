...
- [[https://angular.dev/guide/directives/directive-composition-api][Guida ufficiale Angular]]
- [[http://localhost:4200/directive-composition][Playground]]
...
- [[http://localhost:4200/lazy-loading][Playground]]
...
- ✔ *Migliora le performance* riducendo il bundle iniziale dell’app.
- ✔ *Evita dipendenze inutili* quando un componente non è sempre necessario.
- ✔ *Carica componenti solo quando servono*, ad esempio modali, tabelle avanzate o widget dinamici.

Prima di Angular 17, per creare un componente dinamico si utilizzava =ViewContainerRef= e =ComponentFactoryResolver=. Ora Angular fornisce un metodo più semplice ed efficiente.
...
- [[http://localhost:4200/lifecycle-hooks][Playground]]
...
[[./docs/during-initialization-lifecycle.png]]
...
- [[http://localhost:4200/deferred][Playground]]

** Introduzione

Il deferred loading (caricamento differito) è una tecnica che consente di caricare parti di un'applicazione solo quando sono necessarie, migliorando le prestazioni iniziali.
Angular introduce la direttiva ~@defer~ a partire dalla versione 17 per implementare facilmente questa funzionalità.
Questa direttiva è particolarmente utile per ottimizzare il caricamento di componenti pesanti, moduli o risorse che non sono necessari immediatamente.

** Utilizzo di @defer

La direttiva ~@defer~ permette di ritardare il caricamento di un blocco di template fino al verificarsi di una condizione specifica,
come l'interazione dell'utente o il raggiungimento di un determinato stato.
...
Angular supporta anche il prefetching delle risorse, che consente di caricare in anticipo i componenti o i moduli necessari,
senza renderli immediatamente visibili. Questo è utile per migliorare l'esperienza utente quando si prevede che una risorsa sarà presto necessaria.
...
* Template literals (Novità dalla v19.2)

- [[https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals][MDN]]
# - [[TODO][Guida ufficiale Angular]]
# - [[][Playground]]

Dalla versione *Angular 19* è possibile utilizzare l'interpolazione delle stringhe tramite backtick all'interno del template del componente.
...
@Component({
  ...
  template: `
    <ul
      [attr.title]="user.name + ' ' + user.lastname"
      title="{{ user.name }} + "" + {{ user.lastname }}"
    >
      <li>{{ user.name }}</li>
      <li>{{ user.lastname }}</li>
      <li>Descrizione: {{ user.name }} {{ user.lastname }} ({{ user.age }})</li>
    </ul>
  `,
})
...
@Component({
  selector: 'app-foo'
  template: `
    <ul title=`${user.name} ${user.lastname}`>
      <li>{{ user.name }}</li>
      <li>{{ user.lastname }}</li>
      <li>{{ `Descrizione: ${user.name} ${user.lastname} (${user.age})` }}</li>
    </ul>
...
** Introduzione
In Angular, la comunicazione tra componenti è tradizionalmente gestita attraverso gli *input* e gli *output*. Con l'introduzione dei *Signal* in Angular 16 e l'evoluzione delle API nelle versioni successive, Angular ha introdotto un nuovo approccio per gestire gli input basato sui Signal, che sostituisce o integra il classico decoratore ~@Input~. Allo stesso modo, dalla versione 17, anche la gestione degli *output* ha visto un'evoluzione, introducendo la funzione ~output()~ che semplifica l'emissione di eventi.

** Il vecchio approccio: @Input() e @Output()
Prima di Angular 17, gli input venivano dichiarati utilizzando il decoratore ~@Input~ e gli output con il decoratore ~@Output~. Questo approccio è ancora valido, ma è meno integrato con il sistema reattivo di Angular basato sui Signal.
...
@Component({
  ...
  template: `
    <p>{{ message }}</p>
    <button (click)="notify.emit()">Notifica</button>
  `,
})
export class ChildComponent {
  @Input() message: string = ''; // Input tradizionale
  @Output() notify = new EventEmitter<void>(); // Output tradizionale
}
#+end_src

*** Limitazioni di @Input() e @Output()
- *Mancanza di reattività negli input:* Gli input tradizionali non sono reattivi di default. Se il valore cambia, il componente non viene automaticamente aggiornato a meno che non si utilizzi il change detection.
- *Verbosità:* Richiede l'uso di decoratori e di ~EventEmitter~ per la gestione degli eventi.

** Il nuovo approccio: input() e output() basati su Signal
A partire da Angular 17, è possibile utilizzare la funzione ~input()~ per dichiarare input reattivi basati sui Signal e la funzione ~output()~ per semplificare l'emissione di eventi.
...
@Component({
  ...
  template: `
    <p>{{ message() }}</p>
    <button (click)="notify()">Notifica</button>
  `,
})
export class ChildComponent {
  message = input<string>(''); // Input basato su Signal
  notify = output<void>(); // Output semplificato
}
#+end_src

*** Vantaggi di input() e output()
...
- *Output semplificati:* Non è più necessario creare un ~EventEmitter~, poiché ~output()~ gestisce direttamente l'emissione degli eventi.
- *Proprietà avanzate:* Supporta proprietà come ~required~ e ~transform~ per gli input.
...
** Confronto tra @Input()/@Output() e input()/output()

| *Caratteristica*             | *@Input()*      | *input()*        |
|------------------------------|--------------------------|--------------------------|
| *Reattività*                 | No                       | Sì                       |
| *Sintassi*                   | Decoratori               | Funzioni                 |
...
| *Caratteristica*             | *@Output()*      | *output()*        |
|------------------------------|--------------------------|--------------------------|
| *Reattività*                 | No                       | Sì                       |
| *Sintassi*                   | Decoratori               | Funzioni                 |
| *Integrazione con Signal*    | No                       | No                       |
| *Output semplificati*        | No, usa EventEmitter     | Sì, ~output()~ gestisce l'evento |
| *Proprietà avanzate*         | No                       | Sì (~alias~)  |

** Conclusione
La transizione da ~@Input()~ a ~input()~ e da ~@Output()~ a ~output()~ rappresenta un'evoluzione significativa nella gestione della comunicazione tra componenti in Angular. Gli input basati su Signal offrono maggiore reattività, una sintassi più semplice, una migliore integrazione con il sistema di change detection e nuove proprietà avanzate come ~required~ e ~transform~. Allo stesso modo, la funzione ~output()~ semplifica la gestione degli eventi eliminando la necessità di utilizzare ~EventEmitter~, rendendo il codice più pulito e conciso.

-----

* Two-Way Binding con Model Inputs (Novità dalla v17)

- [[https://angular.dev/guide/templates/two-way-binding][Guida ufficiale Angular]]
# - [[][Playground]]

** Introduzione
Angular 17 introduce una nuova funzionalità chiamata *Model Inputs*, che semplifica il two-way binding senza dover utilizzare la sintassi ~[(ngModel)]~. Questa funzionalità è particolarmente utile per sincronizzare lo stato tra un componente padre e un componente figlio in modo più intuitivo e integrato con i Signal.

** Utilizzo di Model Inputs
Per utilizzare i Model Inputs, è sufficiente definire una proprietà con la funzione ~model()~ nel componente figlio. Questo permette di sincronizzare automaticamente il valore tra il componente padre e il componente figlio.

** Confronto con il vecchio approccio

Prima di Angular 17, il two-way binding richiedeva l’utilizzo di ~@Input()~ e ~@Output()~. Il nuovo approccio semplifica notevolmente il codice.

*** Vecchio approccio con ~@Input()~ e ~@Output()~
#+begin_src typescript
...
** Esempio di Counter con Model Input

*** Componente Padre (ParentComponent)
#+begin_src typescript
...
*** Componente Figlio (ChildComponent)
#+begin_src typescript
import { Component, model } from '@angular/core';
...
export class CounterComponent {
  count = model<number>(0); // Model Input per il two-way binding
...
** Vantaggi
- *Sintassi semplificata:* Elimina la necessità di utilizzare ~@Input()~ e ~@Output()~ per il two-way binding.
...
- *Gestione più semplice dei valori iniziali:* I valori iniziali possono essere passati direttamente al ~model()~.

** Conclusione
I Model Inputs rappresentano un'evoluzione del two-way binding in Angular, rendendo la sincronizzazione tra componenti più semplice e moderna. Il passaggio dal vecchio approccio con ~@Input()~ e ~@Output()~ a ~model()~ semplifica il codice, riducendo la verbosità e migliorando la leggibilità complessiva.

-----

* View Queries (ViewChild / ViewChildren)

- [[https://angular.dev/guide/components/queries][Guida ufficiale Angular]]
# - [[][Playground]]

** Introduzione
Le *view queries* permettono di accedere a elementi del DOM o a componenti/direttive all'interno del template di un componente. Angular fornisce due approcci per questo scopo: i decoratori ~@ViewChild~ e ~@ViewChildren~, e l'uso dei ~Signal Queries~ introdotti di recente.

** ViewChild
Il decoratore ~@ViewChild~ permette di ottenere un riferimento a un singolo elemento, componente o direttiva nel template.

*** Esempio di ViewChild con Decoratori
#+begin_src typescript
...
*** Esempio di ViewChild con Signals
#+begin_src typescript
...
export class ParentComponent {
  ...
  constructor() {
    // childComponent sarà undefined finché il componente non sarà inizializzato
...
*** Esempio di ViewChildren con Decoratori
#+begin_src typescript
...
*** Esempio di ViewChildren con Signals
#+begin_src typescript
...
- *Reattività*: L'uso dei signals consente di sfruttare la reattività di Angular, aggiornando automaticamente le view queries al mutare dei riferimenti, senza necessità di lifecycle hooks come ~ngAfterViewInit~.
- *Sintassi più semplice*: Con ~viewChild~ e ~viewChildren~ si evita la necessità di decoratori e di gestione esplicita del ciclo di vita, riducendo il boilerplate.
- *Limitazioni*: I decoratori tuttavia offrono ancora vantaggi in situazioni in cui si necessita di opzioni avanzate come ad esempio ~{ static: true }~, utili per l'accesso a elementi già presenti all'inizializzazione del componente.

** Conclusione
I ~Signal Queries~ rappresentano un'evoluzione che semplifica il codice e introduce reattività nelle view queries. Tuttavia, i decoratori rimangono utili per scenari complessi che richiedono un controllo più fine del ciclo di vita.
...
** Introduzione
Le *content queries* permettono di accedere a elementi o componenti proiettati all'interno di un componente utilizzando ~<ng-content>~. Angular fornisce due approcci per questo scopo: i decoratori ~@ContentChild~ e ~@ContentChildren~, e l'uso dei ~Signal Queries~.

** ContentChild
Il decoratore ~@ContentChild~ permette di ottenere un riferimento a un singolo elemento o componente proiettato.

*** Esempio di ContentChild con Decoratori
#+begin_src typescript
...
*** Esempio di ContentChild con Signals
#+begin_src typescript
...
export class ParentComponent {
  projectedElement = contentChild('projectedContent');

  constructor() {
    // projectedElement sarà undefined finché il contenuto non sarà inizializzato
    console.log(this.projectedElement()); // undefined
  }
...
*** Esempio di ContentChildren con Decoratori
#+begin_src typescript
...
export class ParentComponent implements AfterContentInit {
  @ContentChildren(ChildComponent, { descendants: true }) projectedComponents!: QueryList<ChildComponent>;
...
*** Esempio di ContentChildren con Signals
#+begin_src typescript
...
export class ParentComponent {
  projectedComponents = contentChildren(ChildComponent);
}
#+end_src

** Differenze e Vantaggi

- *Reattività*: Come per le view queries, i signals permettono l'aggiornamento automatico del riferimento a elementi proiettati senza bisogno di lifecycle hooks.
- *Sintassi più concisa*: L’uso dei ~contentChild~ e ~contentChildren~ riduce il boilerplate.
- *Limitazioni*: A differenza dei decoratori, le Signal Queries non offrono ancora un’opzione equivalente a ~{ descendants: true }~, che permette di selezionare anche gli elementi nidificati.

** Conclusione
I ~Signal Queries~ offrono un modo più elegante e reattivo di lavorare con i contenuti proiettati, riducendo la necessità di lifecycle hooks e rendendo il codice più conciso. Tuttavia, i decoratori rimangono una scelta valida quando serve un controllo più dettagliato, specialmente in contesti complessi dove è necessario selezionare elementi discendenti con l’opzione ~{ descendants: true }~.

-----

* Linked Signal (Novità dalla v19)

** Introduzione
I *Linked Signal* sono una nuova funzionalità introdotta in Angular 19 che permette di creare Signal collegati tra loro, mantenendo una relazione automatica tra stati derivati. Quando il Signal sorgente cambia, tutti i Signal collegati vengono aggiornati automaticamente. Questo è utile per gestire relazioni complesse tra stati in modo reattivo, riducendo la necessità di scrivere codice boilerplate per sincronizzare i valori.

** Utilizzo di LinkedSignal
Per creare un Linked Signal si utilizza la funzione ~linkedSignal~. Questa funzione prende uno o più Signal di input e una funzione che calcola il valore derivato in base a essi.

*** Esempio di Linked Signal
#+begin_src typescript
...
// Linked Signal che calcola il prezzo totale includendo le tasse
const totalPrice = linkedSignal({
  source: () => ({ price: this.basePrice, tax: this.taxRate }),
  computation: (source, previous) => source.price * (1 + source.tax)
});

console.log(totalPrice()); // Output: 120

// Modifica il prezzo base
basePrice.set(200);
console.log(totalPrice()); // Output: 240 (aggiornato automaticamente)

// Modifica l'aliquota fiscale
taxRate.set(0.1);
console.log(totalPrice()); // Output: 220 (aggiornato automaticamente)

// Modifica il prezzo totale
totalPrice.set(456);
console.log(totalPrice()); // Output: 456
#+end_src

** Differenze con ~computed~ ed ~effect~

Sebbene i *Linked Signal* possano sembrare simili ai metodi ~computed~ ed ~effect~, hanno scopi diversi:

- ~computed~: Crea un Signal derivato che viene ricalcolato ogni volta che i Signal di cui dipende cambiano. Tuttavia, il suo valore può essere solo letto e non scritto.
  - *Esempio*:
    #+begin_src typescript
    const basePrice = signal(100);
    const totalPrice = computed(() => basePrice() * 1.2);

    console.log(totalPrice()); // Output: 120

    totalPrice.set(200) // Output: Errore!
    #+end_src

- ~effect~: Serve per eseguire side effect (effetti collaterali) quando uno o più Signal cambiano. Non restituisce alcun valore, ed è utile per eseguire operazioni come il logging o l’aggiornamento di componenti esterni.
  - *Esempio*:
    #+begin_src typescript
    const count = signal(0);
    effect(() => console.log(`Count: ${count()}`));

    count.set(1); // Stampa: "Count: 1"
    #+end_src

- ~linkedSignal~: A differenza di ~computed~, calcola e aggiorna immediatamente il suo valore ogni volta che uno dei Signal di input cambia, e può ricevere più Signal in ingresso. Questo lo rende particolarmente utile quando si ha bisogno di un valore reattivo che si aggiorni automaticamente e immediatamente, senza dipendere da un accesso esplicito come avviene con ~computed~.

** Conclusione
I *Linked Signal* semplificano la gestione di relazioni complesse tra stati, migliorando la reattività e la manutenibilità del codice. Non sostituiscono completamente né gli ~effect~ né i ~computed~, ma forniscono uno strumento aggiuntivo per costruire logiche reattive in modo più chiaro e conciso. La scelta tra ~linkedSignal~, ~computed~ ed ~effect~ dipende dallo scenario: usa ~linkedSignal~ quando vuoi mantenere uno stato derivato aggiornato automaticamente non appena cambiano i suoi Signal sorgente.

-----

* resource(), rxResource(), httpResource()

A partire da Angular 19, sono state introdotte tre nuove API per gestire il caricamento asincrono dei dati in modo reattivo e idiomatico: `resource()`, `rxResource()` e `httpResource()`. Questi strumenti permettono di legare in modo naturale il caricamento dei dati alla UI, sfruttando i Signals e integrandosi perfettamente con `@defer`.

**resource()**

`resource()` è pensata per casi in cui si vuole derivare un valore da una `Promise` (funzione asincrona). Il valore derivato viene esposto come un `Signal` e può essere facilmente utilizzato nei template.
...
Utilizzo nel template con `@defer`:

#+begin_src html
@defer (when user.ready) {
  <p>{{ user()?.name }}</p>
} @loading {
  <p>Caricamento...</p>
} @error {
  <p>Errore: {{ user.error()?.message }}</p>
}
#+end_src

**rxResource()**

`rxResource()` si basa su `Observable` invece che `Promise`, ed è pensato per chi lavora con RxJS. Permette il ricaricamento automatico quando cambiano i parametri di input.
...
const user = rxResource({
  source: () => userId(),
  loader: (id) => inject(HttpClient).get(`/api/users/${id}`)
});
#+end_src

**httpResource()**

`httpResource()` è una forma specializzata di `rxResource()` pensata per lavorare direttamente con `HttpClient` di Angular. Si basa anch’essa su `Signal` e `Observable`, ma integra al meglio la dependency injection.

#+begin_src typescript
const postId = signal(42);

const post = httpResource({
  source: () => postId(),
  url: (id) => `/api/posts/${id}`
});
#+end_src

**Cos'è una ResourceRef**

Ogni chiamata a `resource()`, `rxResource()` o `httpResource()` restituisce un oggetto di tipo `ResourceRef<T>`, dove `T` è il tipo del dato ritornato. Questo oggetto è sia un `Signal<T | undefined>` sia un contenitore di metadati sullo stato corrente della risorsa.

*** Caratteristiche di `ResourceRef`

Un `ResourceRef` espone:

- `()` – legge il valore attuale della risorsa (come un `Signal`)
- `.state()` – restituisce lo stato corrente della risorsa (`'pending' | 'ready' | 'errored'`)
- `.error()` – se presente, restituisce l’oggetto errore
- `.track()` – si usa per attivare il tracciamento reattivo
- `.replay()` – forza il ricaricamento della risorsa
- `.ready` – alias per `state() === 'ready'`
...
#+CAPTION: Tabella di confronto tra resource, rxResource e httpResource
| Funzione        | Input principale       | Tipologia dati     | Reattività | Use-case ideale                             |
|-----------------+------------------------+---------------------+------------+----------------------------------------------|
| `resource()`     | Funzione asincrona      | Promise              | No         | Dati statici, fetch semplice all’avvio       |
| `rxResource()`   | Signal + loader (Obs)   | Observable           | Sì         | Chiamate dipendenti da Signal dinamici       |
| `httpResource()` | Signal + url/template   | Observable (HTTP)    | Sì         | REST API in Angular con parametri dinamici   |

**Conclusione**

Queste API rappresentano un passo importante nella semplificazione della gestione del caricamento dei dati in Angular moderno. Combinate con `@defer`, permettono di ottenere interfacce utente reattive, fluide e ottimizzate, scritte in modo dichiarativo e idiomatico.
